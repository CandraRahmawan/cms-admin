...
if ($this->request->is('post')) {
  ...
  $value_2 = [
    'key1' => empty($data['value_2']) ? '-' : $data['value_2'],
    'key2' => empty($data['value_3']) ? '-' : $data['value_3']
  ];
  
  $value_1 = empty($data['value_1']) ? '-' : $data['value_1'];
  $value_3 = empty($data['image']) ? '-' : $data['image'];;
  if (!empty($id)) {
    $connection->update('plugins_detail', ['value_1' => $value_1, 'value_2' => json_encode($value_2), 'value_3' => $value_3, 'updated_date' => date('Y-m-d H:i:s')], ['plugin_detail_id' => $id]);
  } else {
    $connection->insert('plugins_detail', ['value_1' => $value_1, 'value_2' => json_encode($value_2), 'value_3' => $value_3, 'plugin_id' => $this->request->query('plugin_id')]);
  }
  header("Refresh:0");
}
